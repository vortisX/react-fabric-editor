...
export class EditorEngine {
  ...
  public openGroupEditing(groupId: string): void {
    void this.enterGroupEditing(groupId);
  }

  /** 供 Workspace 命令桥调用：退出当前最内层组合编辑。 */
    ...
    bindEngineEvents({
      ...
      onModified: (event) =>
        handleModified({
          ...
          syncGroupTransform: this.commitGroupTransform,
        }),
      onTextChanged: handleTextChanged,
      onDoubleClick: (event) => this.handleDoubleClick(event.target),
    });
  }
  /** 把高频交互对象加入节流同步流程。 */
  ...
  private handleDoubleClick = (target?: import("fabric").FabricObject): void => {
    if (!this.canvas) return;
    const currentEditingGroupId =
      this.editingGroupStack[this.editingGroupStack.length - 1]?.groupId ?? null;
    if (!target) {
      if (currentEditingGroupId) {
        void this.exitGroupEditing(true);
      }
      return;
    }
    const doubleClickTarget = target as FabricLayerTarget;
    if (!doubleClickTarget.id) return;
    const topLevelTarget = findTopLevelObjectById(this.canvas, doubleClickTarget.id);
    if (!isTopLevelGroupObject(topLevelTarget)) return;
    void this.enterGroupEditing(topLevelTarget.id);
  };
  /** 进入某个组合图层的组内编辑模式，并支持沿嵌套层级继续深入。 */
  private async enterGroupEditing(groupId: string): Promise<void> {
    if (!this.canvas) return;
    this.editingGroupStack = await enterGroupEditing({
      canvas: this.canvas,
      groupId,
      stack: this.editingGroupStack,
    });
  }
  /** 退出当前最内层组编辑，并恢复该组整体边框和控件。 */
  private async exitGroupEditing(shouldSelectGroup: boolean): Promise<void> {
    if (!this.canvas || this.editingGroupStack.length === 0) return;
    this.editingGroupStack = await exitCurrentGroupEditing({
      canvas: this.canvas,
      shouldSelectGroup,
      stack: this.editingGroupStack,
    });
  }
  /** 用当前 Store 中标准化后的 group 快照重建 Fabric 组合对象，避免 scale 累积误差。 */
  private async normalizeGroupObject(
    groupId: string,
    preserveSelection: boolean,
  ): Promise<void> {
    if (
      !this.canvas ||
      this.editingGroupStack.some((entry) => entry.groupId === groupId)
    ) {
      return;
    }
    await normalizeVisibleGroupObject({
      canvas: this.canvas,
      groupId,
      preserveSelection,
    });
