...
3. **模块化 UI**：右侧属性面板按图层类型拆分，复杂交互逻辑优先放入 `*.handlers.ts` 纯 TS 文件。
4. **工作区缩放规则**：当前交互定义为“画布始终保持在工作区中央”，滚轮、右下角按钮与百分比预设均采用中心缩放；缩放动画由 Workspace 层驱动，Engine 只负责显示缩放与 buffer 同步。
